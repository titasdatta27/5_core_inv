...
class ListingAuditMacyController extends Controller
{
    public function listingAuditMacy(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');

        return view('market-places.listingAuditMacy', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewListingAuditMacyData()
    {
        $productMasters = ProductMaster::all();
        $skus = $productMasters->pluck('sku')->unique()->toArray();
        $shopifyData = ShopifySku::whereIn('sku', $skus)->get()->keyBy('sku');
        $dataViewValues = MacyDataView::whereIn('sku', $skus)->pluck('value', 'sku');
    ...
    public function saveAuditToDatabase(Request $request)
    {
        ...
        $ebayDataView = MacyDataView::firstOrNew(['sku' => $sku]);
