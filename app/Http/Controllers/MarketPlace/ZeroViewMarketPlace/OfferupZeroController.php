...
use App\Models\OfferupDataView;
use Illuminate\Http\Request;

class OfferupZeroController extends Controller
{
    public function offerupZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.offerupZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewOfferupZeroData(Request $request)
    {
        ...
        $offerupDataViews = OfferupDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $offerupView = $offerupDataViews[$sku] ?? null;
                $value = $offerupView ? $offerupView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = OfferupDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
