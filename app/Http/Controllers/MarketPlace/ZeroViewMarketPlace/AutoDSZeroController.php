...
use App\Models\AutoDSDataView;
use Illuminate\Http\Request;

class AutoDSZeroController extends Controller
{
    public function autoDSZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.autoDSZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewAutoDSZeroData(Request $request)
    {
        ...
        $autoDSDataViews = AutoDSDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $autoDSView = $autoDSDataViews[$sku] ?? null;
                $value = $autoDSView ? $autoDSView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = AutoDSDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
