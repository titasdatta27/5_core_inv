...
{
    public function mercariWShipZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.mercariWShipZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }
    ...
    public function getViewMercariWShipZeroData(Request $request)
    {
        ...
        $temuMetrics = MercariWShipSheetdata::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $temuDataViews = MercariWShipDataView::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = MercariWShipDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
