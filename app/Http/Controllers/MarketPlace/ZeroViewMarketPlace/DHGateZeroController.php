...
use App\Models\DHGateDataView;
use Illuminate\Http\Request;

class DHGateZeroController extends Controller
{
    public function dhgateZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.dhgateZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewDHGateZeroData(Request $request)
    {
        ...
        $dhgateDataViews = DHGateDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $dhgateView = $dhgateDataViews[$sku] ?? null;
                $value = $dhgateView ? $dhgateView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = DHGateDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
