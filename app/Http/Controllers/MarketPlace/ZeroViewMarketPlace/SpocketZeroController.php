...
use App\Models\SpocketDataView;
use Illuminate\Http\Request;

class SpocketZeroController extends Controller
{
    public function spocketZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.spocketZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewSpocketZeroData(Request $request)
    {
        ...
        $spocketDataViews = SpocketDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $spocketView = $spocketDataViews[$sku] ?? null;
                $value = $spocketView ? $spocketView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = SpocketDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
