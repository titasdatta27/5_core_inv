...
use App\Models\YamibuyDataView;
use Illuminate\Http\Request;

class YamibuyZeroController extends Controller
{
    public function yamibuyZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.yamibuyZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewYamibuyZeroData(Request $request)
    {
        ...
        $yamibuyDataViews = YamibuyDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $yamibuyView = $yamibuyDataViews[$sku] ?? null;
                $value = $yamibuyView ? $yamibuyView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = YamibuyDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
