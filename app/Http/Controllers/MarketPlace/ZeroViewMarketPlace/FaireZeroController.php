...
{
    public function faireZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.faireZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }
    ...
    public function getViewFaireZeroData(Request $request)
    {
        ...
        $temuMetrics = FaireProductSheet::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $temuDataViews = FaireDataView::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = FaireDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
