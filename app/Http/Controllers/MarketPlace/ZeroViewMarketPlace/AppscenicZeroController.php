...
use App\Models\AppscenicDataView;
use Illuminate\Http\Request;

class AppscenicZeroController extends Controller
{
    public function appscenicZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.appscenicZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewAppscenicZeroData(Request $request)
    {
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $appscenicView = $appscenicDataViews[$sku] ?? null;
                $value = $appscenicView ? $appscenicView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = AppscenicDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
