...
use App\Models\PoshmarkDataView;
use Illuminate\Http\Request;

class PoshmarkZeroController extends Controller
{
    public function poshmarkZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.poshmarkZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewPoshmarkZeroData(Request $request)
    {
        ...
        $poshmarkDataViews = PoshmarkDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $poshmarkView = $poshmarkDataViews[$sku] ?? null;
                $value = $poshmarkView ? $poshmarkView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = PoshmarkDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
