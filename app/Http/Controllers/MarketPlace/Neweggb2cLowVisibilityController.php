...
class Neweggb2cLowVisibilityController extends Controller
{
    ...
    public function neweggB2CLowVisibilityView(Request $request)
    {
        ...
        return view('market-places.neweggb2cLowVisibiltyView', [
            'mode' => $mode,
            'demo' => $demo,
    ...
    public function getViewNeweggB2CLowVisibilityData(Request $request)
    {
        ...
        foreach ($productMaster as $row) {
            ...
            // Only include if SESS L30 is between 1 and 100 (remove INV > 0 condition)
            $sessL30 = isset($item['SESS L30']) ? (int) $item['SESS L30'] : 0;
            if ($sessL30 >= 1 && $sessL30 <= 100) {
