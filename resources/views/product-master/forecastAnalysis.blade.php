@extends('layouts.vertical', ['title' => 'Forecast Analysis', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('css')
    ...
    <style>
        ...
        scrollbar-color: black white; /* Firefox */
        }
                    ...
                    <table id="row-callback-datatable" class="table dt-responsive nowrap w-100">
                        ...
                        <tr>
                            <th style="vertical-align: middle; white-space: nowrap;">Sl No</th>
                                ...
                                <th style="vertical-align: middle; white-space: nowrap;">Total month</th>
                                <!-- <th style="vertical-align: middle; white-space: nowrap;">Monthly</th> -->
                                <th style="vertical-align: middle; white-space: nowrap;">MSL</th>
                                <th style="vertical-align: middle; white-space: nowrap;">ORDER given</th>
                                <th style="vertical-align: middle; white-space: nowrap;">2 ORDERED</th>
                                <th style="vertical-align: middle; white-space: nowrap;">Transit</th>
                                <th style="vertical-align: middle; white-space: nowrap;">Approved QTY</th>
                                <th style="vertical-align: middle; white-space: nowrap;">AMZ PRICE</th>
                                <th style="vertical-align: middle; white-space: nowrap;">COMP</th>
                                <th style="vertical-align: middle; white-space: nowrap;">Clink</th>
                                <th style="vertical-align: middle; white-space: nowrap;">OLink</th>
                                <th style="vertical-align: middle; white-space: nowrap;">CBM</th>
                                <th style="vertical-align: middle; white-space: nowrap;">Supplier Tag</th>
                                <!-- <th style="vertical-align: middle; white-space: nowrap;">NR</th> -->
                                <th style="vertical-align: middle; white-space: nowrap;">Req</th>
                                <th style="vertical-align: middle; white-space: nowrap;">@T</th>
                                <th style="vertical-align: middle; white-space: nowrap;">Notes</th>
                            </tr>
                        </thead>
                        <tbody id="table-body">
    ...
    <div class="modal fade" id="monthlyDetailsModal" tabindex="-1" role="dialog" aria-labelledby="monthlyModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body">
                    <div class="d-flex flex-nowrap overflow-auto" style="gap: 1rem;">
                        <div class="card border-left-primary shadow-sm mb-3" style="min-width: 160px;">
                            <div class="card-body">
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                var amzLink = row.OLink || '';
                                var EbayLink = row.OLink || '';

                                if (amzLink || EbayLink) {
                                    return `
                                        <div class="sku-tooltip-container">
                                            <span class="sku-text">${data}</span>
                                            <div class="sku-tooltip">
                                                ${amzLink ? `<div class="sku-link"><a href="${amzLink}" target="_blank" rel="noopener noreferrer">AMZ</a></div>` : ''}
                                                ${EbayLink ? `<div class="sku-link"><a href="${EbayLink}" target="_blank" rel="noopener noreferrer">Ebay</a></div>` : ''}
                                            </div>
                                        </div>
                                    `;
                                } else {
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                if (inv === 0) return '<span class="dil-percent-value gray">0%</span>';

                                const result = Math.round((l30 / inv) * 100); // round to nearest whole number
                                let cssClass = 'gray';

                                if (result > 100) cssClass = 'red';
                                else if (result > 70) cssClass = 'yellow';
                                else if (result > 30) cssClass = 'blue';
                                else if (result >= 0) cssClass = 'green';
                        ...
                        // { data: 'Total', name: 'Total' },
                        {
                            ...
                            name: 'Total month',
                            render: function(data, type, row) {
                                // Round the values and assign them to the respective data attributes
                                const totalMonth = data ? Math.round(parseFloat(data)) : '';
                                const DecValue = row['Dec'] ? Math.round(parseFloat(row['Dec'])) : '';
                                const janValue = row['jan'] ? Math.round(parseFloat(row['jan'])) : '';
                                const febValue = row['feb'] ? Math.round(parseFloat(row['feb'])) : '';
                                const MarValue = row['Mar'] ? Math.round(parseFloat(row['Mar'])) : '';
                                const AprValue = row['Apr'] ? Math.round(parseFloat(row['Apr'])) : '';
                                const MayValue = row['May'] ? Math.round(parseFloat(row['May'])) : '';
                                const JunValue = row['Jun'] ? Math.round(parseFloat(row['Jun'])) : '';
                                const JulValue = row['Jul'] ? Math.round(parseFloat(row['Jul'])) : '';
                                const AugValue = row['Aug'] ? Math.round(parseFloat(row['Aug'])) : '';
                                const SepValue = row['Sep'] ? Math.round(parseFloat(row['Sep'])) : '';
                                const OctValue = row['Oct'] ? Math.round(parseFloat(row['Oct'])) : '';
                                const NovValue = row['Nov'] ? Math.round(parseFloat(row['Nov'])) : '';

                                // Create the icon with the rounded values as data attributes
                                const icon = `
                        ...
                        // },
                        {
                            ...
                            render: function(data, type, row) {
                                // Round the values and assign them to the respective data attributes
                                const mslValue = data ? Math.round(parseFloat(data)) : '';
                                const smslValue = row['S-MSL'] ? Math.round(parseFloat(row['S-MSL'])) : '';
                                const lpMslValue = row['LP * msl'] ? Math.round(parseFloat(row['LP * msl'])) : '';
                                const saleAvgValue = row['SALE AVG'] ? Math.round(parseFloat(row['SALE AVG'])) : '';
                                const saleAnualValue = row['SALE ANNUAL'] ? Math.round(parseFloat(row['SALE ANNUAL'])) : '';
                                ...
                                return `${mslValue}${icon}`;
                            }
                        },
                        { data: 'ORDER given', name: 'ORDER given' },
                        {
                            data: '2 ORDERED',
                            name: '2 ORDERED',
                            render: function(data) {
                        ...
                        { data: 'Transit', name: 'Transit' },
                        { data: 'Approved QTY', name: 'Approved QTY' },
                        {
                            data: 'AMZ PRICE',
                            name: 'AMZ PRICE',
                            render: function(data, type, row) {
                                // Round and extract required data fields
                                const amzPrice = data ? parseFloat(data).toFixed(2) : '';
                                const pftPerc = row['pft%'] ? Math.round(parseFloat(row['pft%'])) : '';
                                const roi = row['ROI'] ? Math.round(parseFloat(row['ROI'])) : '';
                                const ebayPrice = row['EBAY PRICE'] ? parseFloat(row['EBAY PRICE']).toFixed(2) : '';
                                const ebayPftPerc = row['EBAY pft%'] ? Math.round(parseFloat(row['EBAY pft%'])) : '';
                                const ebayRoi = row['EBAY ROI'] ? Math.round(parseFloat(row['EBAY ROI'])) : '';
                                const invAmt = row['Inv Amt'] ? parseFloat(row['Inv Amt']).toFixed(2) : '';
                                const aSaleL30 = row['A SaleL30 amt'] ? parseFloat(row['A SaleL30 amt']).toFixed(2) : '';
                                const eSaleL30 = row['E SaleL30 amt'] ? parseFloat(row['E SaleL30 amt']).toFixed(2) : '';
                                ...
                                return `${amzPrice}${icon}`;
                            }
                        },

                        { data: 'Comp Anal', name: 'Comp Anal' },
                        {
                            data: 'Clink',
                            name: 'Clink',
                            render: function(data) {
                                if (data && data.trim() !== '') {
                                    return `<a href="${data}" target="_blank" title="Click to open"><i class="fas fa-link"></i></a>`;
                                }
                                return '';
                            }
                        },
                        ...
                        // { data: 'CBM', name: 'CBM' },
                        {
                            ...
                            name: 'CBM',
                            render: function(data, type, row) {
                                const cbm = data ? parseFloat(data).toFixed(3) : '';
                                const gwLb = row['GW (LB)'] ?? '';
                                const gwKg = row['GW (KG)'] ?? '';
                                const cbmMsl = row['CBM MSL'] ?? '';
                                const cp = row['CP'] ?? '';
                                ...
                                const icon = `
                                    <i class="fas fa-info-circle cbm-icon"
                                        data-gw_lb="${gwLb}"
                                        data-gw_kg="${gwKg}"
                                        data-cbm_msl="${cbmMsl}"
                                        data-cp="${cp}"
                        ...
                        { data: 'Supplier Tag', name: 'Supplier Tag' },
                        // { data: 'NR', name: 'NR' },
                        {
                            data: 'Req',
                            name: 'Req',
                            render: function(data, type, row, meta) {
                                const options = ['Yes', 'No', 'Later'];
                                let select = `<select class="form-control req-dropdown" data-row-id="${row.id}">`;
                        ...
                        },
                        {
                            data: 'hide',
                            name: 'hide',
                            title: '@T', // Renaming the column header
                            render: function(data, type, row, meta) {
                                const options = ['Need@', '@T', '@Senior'];
                                let select = `<select class="form-control at-dropdown" data-row-id="${row.id}">`;
