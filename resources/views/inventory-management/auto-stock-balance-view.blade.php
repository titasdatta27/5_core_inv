@extends('layouts.vertical', ['title' => 'Auto Stock Balance', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', [
        'page_title' => 'Auto Stock Balance Inventory',
        'sub_title' => 'Auto Stock Balance',
    ])
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6 d-flex align-items-center">
                            <button type="button" class="btn btn-primary" id="openAddWarehouseModal" data-bs-toggle="modal" data-bs-target="#addWarehouseModal">
                                <i class="fas fa-plus me-1"></i> CREATE AUTO STOCK BALANCE
                            </button>
                    ...
                    <div class="modal fade" id="addWarehouseModal" tabindex="-1" aria-labelledby="transferStockModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-xl">
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title"> Auto Stock Balance</h5>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 p-3">
                                        ...
                                        <div class="mb-3">
                                            <label for="added_qty" class="form-label fw-bold">Added Qty</label>
                                            <input type="number" id="added_qty" name="added_qty" class="form-control">
                                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="from_adjust_qty" class="form-label fw-bold">Qty Adj </label>
                                            <input type="number" id="from_adjust_qty" name="from_adjust_qty" class="form-control">
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="to_adjust_qty" class="form-label fw-bold">Qty Adj </label>
                                            <input type="number" id="to_adjust_qty" name="to_adjust_qty" class="form-control">
                                        </div>
                                    </div>

                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="from_adj_dil" class="form-label fw-bold"> Adj Dil% </label>
                                            <input type="number" id="from_adj_dil" name="from_adj_dil" class="form-control" readonly>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="to_adj_dil" class="form-label fw-bold"> Adj Dil% </label>
                                            <input type="number" id="to_adj_dil" name="to_adj_dil" class="form-control" readonly>
                                        </div>  
                                    </div>
...
@section('script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            //                 alert('Error storing auto stock balance.');
            //             }
            //         });
            //     });
            ...
            //     // Auto distribute Added Qty into Adjust Qty
            //     $('input[name="to_adjust_qty"]').on('input', function () {
            //         let addedQty = parseInt($(this).val()) || 0;

            //         // get selected SKUs
            //         let fromSku = $('#from_sku').find('option:selected').val();
            //         let toSku = $('#to_sku').find('option:selected').val();

            //         if (!fromSku || !toSku || addedQty <= 0) return;

            //         // split equally
            //         let splitQty = Math.floor(addedQty / 2);

            //         // helper to calculate adj qty from sku name
            //         function calcAdjQty(sku, qtyShare) {
            //             let pcsMatch = sku.match(/(\d+)\s*pc/i); // match "1pc", "2pcs", "3 pc"
            //             let pcs = pcsMatch ? parseInt(pcsMatch[1]) : 1;
            //             return Math.floor(qtyShare / pcs);
            //         }

            //         // calculate for each sku
            //         let fromAdj = calcAdjQty(fromSku, splitQty);
            //         let toAdj = calcAdjQty(toSku, splitQty);

            //         // set values
            //         $('#from_adjust_qty').val(fromAdj);
            //         $('#to_adjust_qty').val(toAdj);
            //     });

            //     // recalc DIL%
            //     function calcAdjDil(originalDil, originalQty, newAdjQty) {
            //         if (!originalDil || !originalQty || !newAdjQty) return 0;
            //         return ((originalDil * originalQty) / newAdjQty).toFixed(2);
            //     }

            //     let fromDil = parseFloat($('#from_dil_percent').val()) || 0;
            //     let fromQty = parseInt($('#from_available_qty').val()) || 0;
            //     let toDil = parseFloat($('#to_dil_percent').val()) || 0;
            //     let toQty = parseInt($('#to_available_qty').val()) || 0;

            //     let newFromDil = calcAdjDil(fromDil, fromQty, fromAdj);
            //     let newToDil = calcAdjDil(toDil, toQty, toAdj);

            //     $('#from_adj_dil').val(newFromDil);
            //     $('#to_adj_dil').val(newToDil);

            // });

            // $(document).ready(function () {       

            //     // --- existing form submit / select2 / change handlers (keep yours) ---
            //     $('#stockBalanceForm').on('submit', function (e) {
            //         e.preventDefault();
            //         const formData = $(this).serialize();
            //         $.ajax({
            ...
            //     $('#from_sku').select2({ dropdownParent: $('#addWarehouseModal'), placeholder: "Select SKU", allowClear: true });
            //     $('#to_sku').select2({ dropdownParent: $('#addWarehouseModal'), placeholder: "Select SKU", allowClear: true });

            //     // auto-fill fields on select change
            //     $('#from_sku').on('change', function () {
            //         const selected = $(this).find('option:selected');
            //         $('#from_parent_name').val(selected.data('parent') || '');
            //         $('#from_available_qty').val(selected.data('available_qty') || 0);
            //         $('#from_dil_percent').val(selected.data('dil') || 0);
            //         if ($('#added_qty').val()) distributeAndRecalc();
            //     });

            //     $('#to_sku').on('change', function () {
            //         const selected = $(this).find('option:selected');
            //         $('#to_parent_name').val(selected.data('parent') || '');
            //         $('#to_available_qty').val(selected.data('to_available_qty') || 0);
            //         $('#to_dil_percent').val(selected.data('to_dil') || 0);
            //         if ($('#added_qty').val()) distributeAndRecalc();
            //     });
            ...
            //     // --- NEW: event bindings for Added Qty and manual edits to Qty Adj ---
            //     $('#added_qty').on('input', distributeAndRecalc);

            //     // If user manually edits Qty Adj, recompute DILs
            //     $('#from_adjust_qty, #to_adjust_qty').on('input', function () {
            //         recalcDils();
            //     });

            //     // --- Distribution + DIL calculation ---
            ...
            //         // helper to detect pcs per pack
            //         function getPcsCount(sku) {
            //             let pcsMatch = sku.match(/(\d+)\s*pcs?/i);
            //             let pcs = pcsMatch ? parseInt(pcsMatch[1]) : 1;
            //             return pcs > 0 ? pcs : 1;
            //         }
            ...
            //         let halfPieces = Math.floor(addedQty / 2);
            //         let leftover = addedQty % 2;

            //         let fromPieces = halfPieces;
            //         let toPieces = halfPieces;

            //         // assign leftover to 1pc SKU
            //         if (leftover > 0) {
            //             if (fromPcs === 1) {
            //                 fromPieces += leftover;
            //             } else if (toPcs === 1) {
            //                 toPieces += leftover;
            //             } else {
            //                 fromPieces += leftover; // fallback
            //             }
            //         }

            //         // convert pieces → packs
            //         let fromAdj = Math.floor(fromPieces / fromPcs);
            //         let toAdj = Math.floor(toPieces / toPcs);
            ...
            //     // Recalculate Adj DIL% for both sides
            //     function recalcDils() {
            //         let fromDil = parseFloat($('#from_dil_percent').val()) || 0;
            ...
            //         let newFromDil = 0;
            //         if (fromAdj > 0) {
            //             newFromDil = (fromDil * fromAvailable) / fromAdj;
            //             if (!isFinite(newFromDil)) newFromDil = 0;
            //         }
            //         $('#from_adj_dil').val(fromAdj > 0 ? newFromDil.toFixed(2) : '');
            ...
            //         let newToDil = 0;
            //         if (toAdj > 0) {
            //             newToDil = (toDil * toAvailable) / toAdj;
            //             if (!isFinite(newToDil)) newToDil = 0;
            //         }
            //         $('#to_adj_dil').val(toAdj > 0 ? newToDil.toFixed(2) : '');
            //     }
            ...
            $(document).ready(function () {
                ...
                $('#stockBalanceForm').on('submit', function (e) {
                    ...
                    const formData = $(this).serialize();
                    $.ajax({
                        url: '{{ route("autostock.balance.store") }}',
                        method: 'POST',
                        data: formData,
                        success: function (response) {
                            $('#addWarehouseModal').modal('hide');
            ...
            function loadData() {
                $.ajax({
                    url: '/auto-stock-balance-data-list',
                    method: 'GET',
                    headers: {
