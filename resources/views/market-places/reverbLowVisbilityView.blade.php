@extends('layouts.vertical', ['title' => 'Reverb Low View', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Reverb Low View</h4>
                    ...
                    <div class="table-container">
                        <table class="custom-resizable-table" id="reverbLowVisibilty-table">
                            <thead>
                                <tr>
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading Reverb Low data...</div>
                        </div>
                    </div>
                </div>
            </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            // --- Percentage edit code removed ---


            // Cache system
            const reverbLowVisibiltyDataCache = {
                cache: {},
            ...
            window.addEventListener('load', function() {
                reverbLowVisibiltyDataCache.clear();
            });
            ...
            function loadData() {
                showLoader();
                return $.ajax({
                    url: '/reverb/zero-low-visibility/view-data',
                    type: 'GET',
                    dataType: 'json',
            ...
            function renderTable() {
                const $tbody = $('#reverbLowVisibilty-table tbody');
                ...
                const $headers = $('#reverbLowVisibilty-table thead th');
            ...
            window.openModal = function(selectedItem, type) {
                try {
                    ...
                    const cachedData = reverbLowVisibiltyDataCache.get(itemId);
                    ...
                    if (!cachedData) {
                        reverbLowVisibiltyDataCache.set(itemId, itemData);
                    }
            ...
            function setupEditHandlers(modalId) {
                ...
                $(modalElement).on('click', '.edit-icon', function(e) {
                    ...
                    // For Bump/S bump and other editable fields
                    if (contentElement.find('input').length === 0) {
                        const originalValue = contentElement.text().trim();
                        contentElement.data('original-content', originalValue);
                        contentElement.html(
                            `<input type="text" class="form-control form-control-sm bump-edit-input" value="${originalValue}">`
                        );
                        contentElement.find('input').focus();
                    }
                ...
                $(modalElement).on('click', '.save-icon', function(e) {
                    ...
                    if (checkbox.length > 0) {
                        ...
                        updatedValue = contentElement.find('input').length ?
                            contentElement.find('input').val().trim() :
                    ...
                    // Restore pen icon and static text after save
                    icon.html('<i class="fas fa-pen text-primary"></i>')
                        .removeClass('save-icon')
                        .addClass('edit-icon');
                    if (checkbox.length === 0) {
                        contentElement.html(updatedValue);
                    }
                });
            }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (index !== -1) {
                            ...
                            reverbLowVisibiltyDataCache.updateField(slNo, normalizedTitle, data.updates[
                                normalizedTitle]);
                            if (normalizedTitle === 'bump') {
                                reverbLowVisibiltyDataCache.updateField(slNo, 'Bump', data.updates[
                                    normalizedTitle]);
                            }
                            if (normalizedTitle === 's_bump') {
                                reverbLowVisibiltyDataCache.updateField(slNo, 's bump', data.updates[
                                    normalizedTitle]);
                            }
                            if (normalizedTitle === 's_price') {
                                reverbLowVisibiltyDataCache.updateField(slNo, 'SPRICE', data.updates[
                                    normalizedTitle]);
                            }
                            // --- Console log cache data after update ---
                            console.log('Cache after update:', reverbLowVisibiltyDataCache.get(slNo));
                        }
                        ...
                        if (!isCheckbox && contentElement) {
                            ...
                            const cached = reverbLowVisibiltyDataCache.get(slNo);
            ...
            function initResizableColumns() {
                const $table = $('#reverbLowVisibilty-table');
            ...
            $('#saveReasonActionBtn').on('click', function() {
                ...
                if (sku) {
                    $.ajax({
                        url: '/reverb-low-visibility/reason-action/update',
                        method: 'POST',
                        data: {
                            sku: sku,
                            reason: reason,
