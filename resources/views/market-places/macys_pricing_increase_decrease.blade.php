...
@section('script')
        ...
        $(document).ready(function() {
                ...
                return $.ajax({
                    ...
                    success: function(response) {
                        if (response && response.product_master_data) {
                            const rawData = response.product_master_data;

                            // No filtering — include all rows regardless of INV or VIEWS
                            ...
                            tableData = validData.map((item, index) => {
                                ...
                                return {
                                    ...
                                    'LMP 1': sheet['LMP 1'] || 0,
                                    'C link': sheet['C link'] || 0,
                                    'lmp2': sheet['lmp2'] || 0,
                                    'c link 2': sheet['c link 2'] || 0,
                                };
                            });
            ...
            function renderTable() {
                ...
                filteredData.forEach(item => {
                    ...
                    // ✅ SPRICE (rounded to nearest dollar)
                    $row.append($('<td>').html(
                        item.SPRICE !== null && !isNaN(parseFloat(item.SPRICE)) ?
                        `
    <span class="sprice-value text-success">
        $${Math.round(parseFloat(item.SPRICE))}
    </span>
    <button class="btn btn-sm btn-outline-primary ms-2 view-sprice-btn" data-id="${item.id}" title="Edit SPRICE" style="cursor:pointer;" 
    ...
    ` : '0'
                    ...
                    $row.append($('<td>').html(
                        item['C link'] ? `
        <a href="${item['C link']}" target="_blank" title="Open Link">
    ...
    `
                    ...
                    $row.append($('<td>').html(
                        typeof item['lmp2'] === 'number' && !isNaN(item['lmp2']) ?
                        `
                                <span class="dil-percent-value ${getPftColor(item['lmp2'])}">
                                 ${Math.round(item['lmp2'])}
                                </span>
                                ` : ''
                    ));
                    ...
                    $row.append($('<td>').html(
                        item['c link 2'] ? `
        <a href="${item['c link 2']}" target="_blank" title="Open Link">
            <i class="fa fa-link"></i>
        </a>
    ` : `
        <span style="color: red; font-weight: bold;">✘</span>
                ...
                <div class="modal fade" id="pricingModal" tabindex="-1" aria-labelledby="pricingModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content p-3">
                            ...
                            <div class="modal-body">
                                <form id="pricingForm" method="POST" action="{{ url('/macys/save-sprice') }}">
                                    @csrf
