    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function createTableRow(item, index) {
                ...
                if (!item.sku.includes('PARENT')) {
                    // Check if nr_req is 'NR', then show 'NRL' text instead of dropdown
                    if (item.nr_req === 'NR') {
            ...
            $(document).on('change', '.nr-req-dropdown, .listed-dropdown', function() {
                ...
                if (nr_req === 'NR') {
                    $(this).css('background-color', '#dc3545').css('color', '#ffffff');
                } else {
                    $(this).css('background-color', '#28a745').css('color', '#ffffff');
                }
            ...
            $(document).on('change', '.nr-req-dropdown', function() {
                ...
                // If NR is selected, replace dropdown with 'NRL' text
                if (nr_req === 'NR') {
                    const $badge = $('<button>').text('NRL').css({
                        'color': 'white',
                        'background-color': '#dc3545',
