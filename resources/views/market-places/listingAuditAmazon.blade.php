@extends('layouts.vertical', ['title' => 'Listing Audit Amazon', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', [
        'page_title' => 'Listing Audit Amazon',
        'sub_title' => 'Amazon',
    ])
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadData() {
                showLoader();
                return $.ajax({
                    url: '/listing_audit_amazon/view-data',
                    type: 'GET',
                    dataType: 'json',
            ...
            function createTableRow(item, index) {
                ...
                if (item.is_parent) {
                    ...
                    // Show SKU with image on hover
                    let skuHtml = '';
                    if (item.image_path) {
                        skuHtml += `
                            <span class="sku-tooltip-container">
                                <span class="sku-link">${item.sku}</span>
                                <span class="sku-tooltip">
                                    <img src="${item.image_path}" alt="SKU Image" class="image-thumbnail" style="max-width:100px;max-height:100px;">
                                </span>
                            </span>
                        `;
                    } else {
                        skuHtml += `<span>${item.sku}</span>`;
                    }
                    $row.append($('<td>').html(skuHtml));
                ...
                $(document).on('change', '.nr-select', function () {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            if (nrValue === 'NR') {
                                filteredData = filteredData.filter(item => item.sku !== sku);
                            } else {
                                // Update NR value in filteredData
            ...
            function initNAEditHandlers() {
                ...
                $(document).on('change', '.na-checkbox', function(e) {
                    ...
                    $.ajax({
                        url: '/listing_audit_amazon/save-na',
                        type: 'POST',
                        data: {
                            sku: sku,
                            field: field,
