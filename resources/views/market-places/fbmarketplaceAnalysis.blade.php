@extends('layouts.vertical', ['title' => 'FB Marketplace', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', ['page_title' => 'FB Marketplace', 'sub_title' => 'Market Place'])
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">FB Marketplace Product Analysis</h4>
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-3 align-items-center justify-content-between">
                        ...
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            <!-- Export Button -->
                            <a href="{{ route('fbmarketplace.analytics.export') }}" class="btn btn-success">
                                <i class="fas fa-file-export me-1"></i> Export Live/Listings
                            </a>

                            <!-- Import Button -->
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                data-bs-target="#fbmarketplaceImportModal">
                                <i class="fas fa-file-import me-1"></i> Import Live/Listings
                            </button>
                        </div>
                    </div>

                    <div class="modal fade" id="fbmarketplaceImportModal" tabindex="-1" aria-labelledby="fbmarketplaceImportModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog">
                            <form action="{{ route('fbmarketplace.analytics.import') }}" method="POST"
                                enctype="multipart/form-data" class="modal-content" id="fbmarketplaceImportForm">
                                @csrf
                                <div class="modal-header">
                                    <h5 class="modal-title" id="fbmarketplaceImportModalLabel">Import Fb Marketplace Data</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <!-- File Input -->
                                    <div class="mb-3">
                                        <label for="fbmarketplaceExcelFile" class="form-label">Select Excel File</label>
                                        <input type="file" class="form-control" id="fbmarketplaceExcelFile" name="excel_file"
                                    ...
                                    <div class="alert alert-info">
                                        <small>
                                            ...
                                            Download the <a href="{{ route('fbmarketplace.analytics.sample') }}"
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading Fb Marketplace data...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <i class="fas fa-check-circle text-success tooltip-icon conversion-view-trigger ms-2"
        ...
        data-item='${JSON.stringify(item.raw_data)}'></i>`
            ...
            $(document).on('change', '.listed-checkbox, .live-checkbox', function() {
                ...
                $.ajax({
                    url: '/fbmarketplace/update-listed-live',
                    method: 'POST',
                    data: {
                        sku: sku,
                        field: field,
