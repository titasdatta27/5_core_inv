@extends('layouts.vertical', ['title' => 'Listing CVR Analysis'])
...
@section('content')
    @include('layouts.shared.page-title', [
        'page_title' => 'Listing CVR Analysis',
        'sub_title' => 'Listing CVR Analysis',
    ])
        ...
        <div class="col-12">
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <a href="{{ route('pricing-master.export') }}" class="btn btn-success" title="Export to Excel">
                        <i class="mdi mdi-download me-1"></i> Export Data
                    </a>

                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#importL90Modal" title="Import Site-wise L90 Data">
                        <i class="mdi mdi-upload me-1"></i> Import Site L90
                    </button>
                    </div>
...
{{-- Import Site-wise L90 Data Modal --}}
<div class="modal fade" id="importL90Modal" tabindex="-1" aria-labelledby="importL90ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="importL90ModalLabel">
                    <i class="mdi mdi-upload"></i> Import Site-wise L90 Sales & Views
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-info">
                    <h6><i class="mdi mdi-information"></i> Import Instructions:</h6>
                    <ul class="mb-0">
                        <li>Download the sample Excel file with all SKUs pre-filled</li>
                        <li>Fill in the L90 sales and views data for each marketplace</li>
                        <li>Upload the completed file to update Product Master table</li>
                        <li>All fields except SKU are optional</li>
                    </ul>
                </div>

                <div class="mb-3">
                    <a href="{{ route('pricing-master.download-site-l90-sample') }}" class="btn btn-outline-primary">
                        <i class="mdi mdi-download"></i> Download Sample Excel
                    </a>
                </div>

                <form id="importL90Form" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label for="l90File" class="form-label">Select Excel File</label>
                        <input type="file" class="form-control" id="l90File" name="file" accept=".xlsx,.xls" required>
            ...
            columns: [{
...

               {
...

                {
                    ...
                    formatter: function(cell) {
                        const data = cell.getRow().getData();
                        // Calculate sum of all sales from sales JSON
                        let totalSales = 0;
                        if (data.sales) {
                            Object.values(data.sales).forEach(val => {
                                totalSales += parseFloat(val) || 0;
                            });
                        }

                        return `<button class="btn btn-outline-success rounded-pill px-3 text-success" style="cursor:default !important; background-color: #fff !important">
                            <i class="bi bi-cart me-1"></i>${Math.round(totalSales)}
                        </button>`;
                    }
                },
          ...
          const rows = [
            ...
            <table class="table table-sm table-bordered align-middle sortable-table">
                ...
                <tr>
                    ...
                    <th data-sort="number">L90 Sales <i class="bi bi-arrow-down-up"></i></th>
                    <th data-sort="number">L90 Views <i class="bi bi-arrow-down-up"></i></th>
                ...
                html += `
                 <tr>
                    ...
                    <td>
                        <div class="value-indicator">
                            ${(() => {
                                // Get L90 sales from the sales JSON data
                                ...
                                const channelKey = channelMap[r.prefix];
                                return salesData[channelKey] ? Math.round(parseFloat(salesData[channelKey])) : "-";
                            })()}
                        </div>
                    </td>
                    <td>
                        <div class="value-indicator">
                            ${(() => {
                                // Get L90 views from the views JSON data
                                ...
                                const channelKey = channelMap[r.prefix];
                                return viewsData[channelKey] ? Math.round(parseFloat(viewsData[channelKey])) : "-";
                            })()}
                        </div>
                    </td>
...
// Import L90 Data functionality
$('#startImportBtn').on('click', function() {
    const form = $('#importL90Form')[0];
    const fileInput = $('#l90File')[0];
    ...
    $.ajax({
        url: '{{ route("pricing-master.import-site-l90") }}',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
            ...
            if (response.success) {
                $('#importResult').html(`
                    <div class="alert alert-success">
                        <h6><i class="mdi mdi-check-circle"></i> Import Successful!</h6>
                        <p class="mb-0">
                            Total SKUs: ${response.total}<br>
                            Successfully Imported: ${response.imported}<br>
                ...
                $('#importResult').html(`
                    <div class="alert alert-danger">
                        <h6><i class="mdi mdi-alert-circle"></i> Import Failed</h6>
        ...
        error: function(xhr) {
            ...
            const errorMsg = xhr.responseJSON?.message || 'An error occurred during import';
            $('#importResult').html(`
                <div class="alert alert-danger">
                    <h6><i class="mdi mdi-alert-circle"></i> Import Error</h6>
