@extends('layouts.vertical', ['title' => 'KW Ebay', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    ...
    @include('layouts.shared/page-title', [
        'page_title' => 'KW EBay',
        'sub_title' => 'Ebay master Analysis',
    ])
...
@section('script')
    ...
    <script>
        const campaignSheetData = @json($sheetData['ebay_kw_l30']);
        const ebayKwL1 = @json($sheetData['ebay_kw_l1']);
        ...
        function findSheetL1BySKU(sku) {
            if (!sku || !Array.isArray(ebayKwL1)) return null;

            const normalizedSKU = sku.trim().toUpperCase();

            return ebayKwL1.find(item => {
                const campaignName = (item['Campaign name'] || '').trim().toUpperCase();
                return campaignName === normalizedSKU;
            });
        }
            ...
            function renderTable(data) {
                ...
                data.forEach(item => {
                    if (item.Parent && !String(item.SKU).toUpperCase().includes('PARENT')) {
                        ...
                        // Parse as numbers, treat missing as 0
                ...
                displayItems.forEach(item => {
                    ...
                    if (item.SKU && item.SKU.toUpperCase().includes('PARENT')) {
                        ...
                        // Show totals for this parent in INV and OV L30 columns, rest from item
                        const totals = parentTotals[item.Parent] || {
                            inv: 0,
                            ovl30: 0
                        };
                        let dil = '-';
                        if (totals.inv && !isNaN(totals.ovl30) && totals.inv !== 0) {
                            // dil = ((totals.ovl30 / totals.inv) * 100).toFixed(1) + '%';
                            dil = Math.round((totals.ovl30 / totals.inv) * 100) + '%';
                        }
                        let sku = item.sku?.trim().toUpperCase() || '';
                        let campaignData = campaignSheetData.find(c =>  
                            (c['Campaign name'] || '').trim().toUpperCase() === sku
                        );

                        const sheet = findSheetBySKU(item.SKU || '');
                        const impl30 = campaignData ? campaignData['Impressions'] : '-';
                        const clicksl30 = campaignData ? campaignData['Clicks'] : '-';
                        const conversionl30 = sheet?.conversionl30 ?? '-';


                        // const flags = dbFlags[sku] || {};
                        // const raChecked = flags.ra == 1 ? 'checked' : '';
                        // console.log("RA:", raChecked);
                        ...
                        row.innerHTML += `
                            ...
                            <td><input type="checkbox" class="flag-checkbox" data-sku="${sku}" data-field="ra"></td>
                            <td><input type="checkbox" class="flag-checkbox" data-sku="${sku}" data-field="nra"></td>
                            <td><input type="checkbox" class="flag-checkbox" data-sku="${sku}" data-field="running"></td>
                            <td><input type="checkbox" class="flag-checkbox" data-sku="${sku}" data-field="to_pause"></td>
                            <td><input type="checkbox" class="flag-checkbox" data-sku="${sku}" data-field="paused"></td>
                            <td>${impl30}</td>
                            <td class="text-center">${clicksl30}</td>
                            <td class="text-center">${conversionl30}</td>
                            <td class="text-center">${formatNumber(item.frght, 2)}</td>
                            <td class="text-center">${escapeHtml(item.ship) || '-'}</td>
                            <td class="text-center">${escapeHtml(item.label_qty) || '0'}</td>
                            <td class="text-center">${formatNumber(item.wt_act || 0, 2)}</td>
                            <td class="text-center">${formatNumber(item.wt_decl || 0, 2)}</td>
                            <td class="text-center">${formatNumber(item.l || 0, 2)}</td>
                            <td class="text-center">${formatNumber(item.w || 0, 2)}</td>
                            <td class="text-center">${formatNumber(item.h || 0, 2)}</td>
                            <td class="text-center">${formatNumber(item.cbm, 4)}</td>
                    ...
                    const sheet = findSheetBySKU(item.SKU || '');
                    const sheetL1 = findSheetL1BySKU(item.SKU || '');
                    
                    const impl30 = sheet?.Impressions ?? '-';
                    const clicksl30 = sheet?.Clicks ?? '-';
                    const spendl30 = sheet ? sheet['Ad fees'] : '-';
                    const adSalesl30 = sheet?.Sales ?? '-';
                    const adSoldl30 = sheet ? sheet['Sold quantity'] : '-';
                    const bgtCrnt = sheet ? sheet['Daily budget'] : '-';
                    const cpcl1 = sheetL1 ? sheetL1['Average cost per click'] : '-';
                ...
                setupParentNavigation(); 
                // console.log(" Buttons setup done");
