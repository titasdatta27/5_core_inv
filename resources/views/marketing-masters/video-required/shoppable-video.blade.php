@extends('layouts.vertical', ['title' => 'Shoppable Video Required'])
...
@section('content')
@include('layouts.shared.page-title', ['page_title' => 'Shoppable Video Required', 'sub_title' => 'Shoppable Video Required'])
    ...
    <div class="col-12">
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <div id="shoppable-video-req"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const table = new Tabulator("#shoppable-video-req", {
            index: "Sku",
            ajaxURL: "/shoppable-video-req/view-data",
            ajaxConfig: "GET",
            layout: "fitColumns",
            pagination: true,
            paginationSize: 50,
        ...
        table.on("cellEdited", function(cell) {
            ...
            const payload = {
                sku: rowData.Sku,
                value: {
                    TikTokVideoLink: rowData.TikTokVideoLink || '',
                }
            };

            fetch("/shoppable-video-req/save", {
                method: "POST",
                headers: {
        ...
        document.addEventListener("change", function (e) {
            const isEditable = e.target.classList.contains("editable-select");
            if (!isEditable) return;

            const tableInstance = Tabulator.findTable("#shoppable-video-req")[0];
            ...
            // 🔒 Prevent "REQ" in Ads if TikTokVideoLink is empty
            if (field === "Ads" && value === "REQ" && (!rowData.TikTokVideoLink || rowData.TikTokVideoLink.trim() === "")) {
                alert("Please update TikTok Video Link before setting Ads to REQ.");
                e.target.value = "NR";
                return;
            }
