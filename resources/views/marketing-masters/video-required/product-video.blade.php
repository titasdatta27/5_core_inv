@extends('layouts.vertical', ['title' => 'Posted Video Required'])
...
@section('content')
@include('layouts.shared.page-title', ['page_title' => 'Posted Video Required', 'sub_title' => 'Posted Video Required'])
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <div class="mb-3 d-flex flex-wrap align-items-center gap-3">
                    <form action="{{ route('video.import') }}" method="POST" enctype="multipart/form-data" hidden>
                        @csrf
                        <input type="file" name="excel_file" accept=".xlsx, .xls" required>
                        <button type="submit" class="btn btn-success">Import Sheet</button>
                    </form>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
            ...
            columns: [
                    ...
                    formatter: function (cell) {
                        ...
                        return `
                            <select class="form-select form-select-sm editable-select"
                                ...
                                <option value="Posted Video" ${value === 'Posted Video' ? 'selected' : ''}>Posted Video</option>
                                <option value="Assembly Video" ${value === 'Assembly Video' ? 'selected' : ''}>Assembly Video</option>
                                <option value="3D Video" ${value === '3D Video' ? 'selected' : ''}>3D Video</option>
                                <option value="Benifits Video" ${value === 'Benifits Video' ? 'selected' : ''}>Benifits Video</option>
                                <option value="DIY Video" ${value === 'DIY Video' ? 'selected' : ''}>DIY Video</option>
                                <option value="Shoppable Video" ${value === 'Shoppable Video' ? 'selected' : ''}>Shoppable Video</option>
                            </select>
                        `;
                    }
                ...
                },
                {
                    ...
                    titleFormatter: function() {
                        return `<div>
                            Avl<br>
                            <span id="total-avl-header" style="font-size:13px;color:white;font-weight:600;"></span>
                        </div>`;
                    },
                    formatter: function (cell) {
                        const row = cell.getRow();
                ...
                },
                {
                    ...
                    titleFormatter: function () {
                        return `<div>
                            Product Video <i class='fa fa-link'></i>
                        </div>`;
                    },
                    hozAlign: "center",
                },
        ...
        table.on("cellEdited", function(cell) {
            ...
            const payload = {
                ...
                value: {
                    ...
                    Avl: rowData.Avl || false,
                    Approved: rowData.Approved || false,
                    Assigned: rowData.Assigned || '',
                    ProductVideo: rowData.ProductVideo || '',
                    Remarks: rowData.Remarks || ''
                }
            };

            fetch("/video-posted/save", {
                method: "POST",
                headers: {
        ...
        function updateTotalInvAndL30(table) {
            ...
            const totalAvl = data.filter(row => row["Avl"] === true).length;
            ...
            document.getElementById("total-avl-header").textContent = totalAvl.toLocaleString();
        ...
        document.addEventListener("change", function (e) {
            ...
            const tableInstance = Tabulator.findTable("#posted-video-req")[0];
            ...
            if (field === "Approved" && value === true && (!rowData.ProductVideo || rowData.ProductVideo.trim() === "")) {
                alert("Please update ProductVideo link first.");
